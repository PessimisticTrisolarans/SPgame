...
pygame.display.set_caption("Tetris_difficulty")
        ...
        for x, cell in enumerate(row):
            if cell:
                pygame.draw.rect(screen, block.color,
                                 ((block.x + x) * GRID_SIZE, (block.y + y) * GRID_SIZE, GRID_SIZE, GRID_SIZE))
...
def get_high_score_from_file():
    try:
        with open('scores_for_difficulty.txt', 'r') as file:
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_r:  # 按R键重新开始游戏
                    ...
                    if event.key == pygame.K_LEFT and not check_collision(
                        ...
                        while not check_collision(Block(current_block.shape, current_block.color, current_block.x, current_block.y + 1),
                                                  grid):
                            current_block.y += 1
        ...
        if not game_over and not paused:
            ...
            if check_collision(current_block, grid):
                ...
                if score > high_score:
                    high_score = score
                current_block = Block(x=random.randint(0, GRID_WIDTH - len(current_block.shape[0])), y=0) #生成新的方块时，同样使用随机x坐标
                if check_collision(current_block, grid):
                    game_over = True
